...
type Role struct {
	...
	RoleDescription string `boil:"role_description" json:"role_description" toml:"role_description" yaml:"role_description"`
...
var RoleWhere = struct {
	...
	RoleDescription whereHelperstring
	Enabled         whereHelperstring
	BankID          whereHelperint
}{
	RoleID:          whereHelperint{field: `role_id`},
	RoleDescription: whereHelperstring{field: `role_description`},
...
var RoleRels = struct {
	RoleRoleResources string
}{
	RoleRoleResources: "RoleRoleResources",
}

// roleR is where relationships are stored.
type roleR struct {
	RoleRoleResources RoleResourceSlice
}
...
var (
	roleColumns               = []string{"role_id", "role_description", "enabled", "bank_id"}
	roleColumnsWithoutDefault = []string{"role_id", "role_description", "enabled", "bank_id"}
...
// RoleRoleResources retrieves all the RoleResource's RoleResources with an executor via role_id column.
func (o *Role) RoleRoleResources(mods ...qm.QueryMod) roleResourceQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`RoleResources`.`role_id`=?", o.RoleID),
	)
...
// LoadRoleRoleResources allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (roleL) LoadRoleRoleResources(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(qm.From(`RoleResources`), qm.WhereIn(`role_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.RoleRoleResources = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.RoleID == foreign.RoleID {
				local.R.RoleRoleResources = append(local.R.RoleRoleResources, foreign)
				if foreign.R == nil {
					foreign.R = &roleResourceR{}
				}
...
// AddRoleRoleResources adds the given related objects to the existing relationships
// of the Role, optionally inserting them as new records.
// Appends related to o.R.RoleRoleResources.
// Sets related.R.Role appropriately.
func (o *Role) AddRoleRoleResources(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RoleResource) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.RoleID, rel.RoleResourceID}
	...
	if o.R == nil {
		o.R = &roleR{
			RoleRoleResources: related,
		}
	} else {
		o.R.RoleRoleResources = append(o.R.RoleRoleResources, related...)
	}
...
func FindRole(ctx context.Context, exec boil.ContextExecutor, roleID int, selectCols ...string) (*Role, error) {
	...
	query := fmt.Sprintf(
		"select %s from `Roles` where `role_id`=?", sel,
	)
...
func RoleExists(ctx context.Context, exec boil.ContextExecutor, roleID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `Roles` where `role_id`=? limit 1)"

	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, sql)
		fmt.Fprintln(boil.DebugWriter, roleID)
	}
